...
function render(){$('count').textContent=`${total} Lincoln records`;$('pageInfo').textContent=`Page ${page} of ${Math.max(1,Math.ceil(total/limit))}`;$('rows').innerHTML=records.length?records.map((r,i)=>`<tr><td>${fmt(r.created_at)}</td><td>${esc(r.et_number)}</td><td>${esc(r.driver_name)}</td><td>${esc(r.carrier_name)}</td><td>${esc(r.equipment_no)}</td><td>${esc(r.load_type_group||r.entry_task)}</td><td><span class="pill">${esc(r.direction)}</span></td><td>${esc(r.load_no||r.receipt_id||r.po_no||r.reference_no)}</td><td>${esc(r.customer)}</td><td>${esc(r.door_assignment)}</td><td>${esc(r.assigned_to||'')}</td><td>${statusPill(r.assignment_status)}</td><td><button onclick="detail(${i})">Review</button></td></tr>`).join(''):`<tr><td colspan="13" class="empty">No Lincoln check-ins found.</td></tr>`}
