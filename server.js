...
const WMS_FACILITY_ID = process.env.WMS_FACILITY_ID || "LT_ORG-7759";
...
const SMTP_FROM = process.env.SMTP_FROM || SMTP_USER || "Fontana Driver Check-In <no-reply@unisco.com>";
...
function formatNotificationLines(etNumber, payload) {
  ...
  return [
    `A driver has completed check-in at Fontana (LT_ORG-7759).`,
    ``,
    `ET#: ${etNumber || ""}`,
...
async function sendCheckinEmailNotification(etNumber, payload) {
  ...
  const trip = payload.tripInfo || {};
  const subjectParts = ["Fontana Driver Check-In", etNumber];
...
async function sendStoredCheckinEmailNotification(record = {}) {
  ...
  const subject = ["Fontana Driver Check-In", etNumber, driverName].filter(Boolean).join(" - ");
  const lines = [
    "A driver has completed check-in at Fontana (LT_ORG-7759).",
    "",
  ...
  const html = `<div style="font-family:Arial,sans-serif;line-height:1.45;color:#111">
    <h2 style="margin:0 0 12px">Fontana Driver Check-In</h2>
